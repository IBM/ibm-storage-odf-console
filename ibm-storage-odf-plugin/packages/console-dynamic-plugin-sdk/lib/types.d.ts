 ...
 */
export declare type ExtensionTypeGuard<E extends Extension> = (e: E) => e is E;
/**
 * Runtime extension interface, exposing additional metadata.
 */
export declare type LoadedExtension<E extends Extension = Extension> = E & {
    pluginID: string;
    pluginName: string;
    uid: string;
};
 ...
 */
export declare type Extension<P extends {} = any> = {
    type: string;
    properties: P;
 ...
 */
export declare type ExtensionDeclaration<T extends string, P extends {}> = Extension<P> & {
    type: T;
};
/**
 * Remote (i.e. webpack container) entry module interface.
 */
export declare type RemoteEntryModule = {
    /**
 ...
 */
export declare type CodeRef<T = any> = () => Promise<T>;
/**
 * Infer resolved `CodeRef` properties from object `O`.
 */
export declare type ResolvedCodeRefProperties<O extends {}> = {
    [K in keyof O]: O[K] extends CodeRef<infer T> ? T : O[K];
};
/**
 * Update existing properties of object `O` with ones declared in object `U`.
 */
export declare type Update<O extends {}, U extends {}> = {
    [K in keyof O]: K extends keyof U ? U[K] : O[K];
} & {};
/**
 * Infer the properties of extension `E`.
 */
export declare type ExtensionProperties<E> = E extends Extension<infer P> ? P : never;
/**
 * Update existing properties of extension `E` with ones declared in object `U`.
 */
export declare type UpdateExtensionProperties<E extends Extension<P>, U extends {}, P = ExtensionProperties<E>> = Update<E, {
 ...
 */
export declare type ResolvedExtension<E extends Extension<P>, P = ExtensionProperties<E>> = LoadedExtension<UpdateExtensionProperties<E, ResolvedCodeRefProperties<P>, P>>;
