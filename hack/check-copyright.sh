...
for file in $(find $PROTECT_ROOT -not -path "*/dist/*" -not -path "*i18next-parser.config.js*" -not -path "*/i18n-scripts/*" -not -path "*/vendor/*" -not -path "*/.git/*" -not -path "*/node_modules/*" -not -path "*/.husky/*" -type f \( -name '*.go' -o -name '*.proto' -o -name '*.sh' -o -name '*.js' -o -name '*.ts' -o -name '*.tsx' \)); do
  if [ ! -s $file ]; then
